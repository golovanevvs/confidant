...
5. Синхронизация e-mail.
6. Клиент отсылает на сервер список ID, сформированный в п.4. Сервер по данным ID возвращает данные групп. Клиент сохраняет их в БД.
7. Клиент формирует данные групп из списка по п.3 (которых нет на сервере). Отсылает их на сервер. Сервер сохраняет их в БД и присылает полученные ID групп. Клиент обновляет ID этих групп у себя в БД (поле `id_on_server`).
...
2. Только создатель группы может добавлять e-mail.
...
5. Добавить для сервера поддержку SQLite.
6. Перейти на применение транзакций.
7. Добавить лицензию MIT.
