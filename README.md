...
Для каждого типа своя форма записи и отображения информации.
...
Эндпоинты синхронизации на сервере доступны только авторизованным клиентам.
...
Если в сервере такой группы ещё нет, то в поле `id_on_server` записывается `-1`.
...
6. Клиент формирует данные групп из списка по п.3 (которых нет на сервере). Отсылает их на сервер. Сервер сохраняет их в БД и присылает полученные ID групп. Клиент обновляет ID этих групп у себя в БД.
...
2. Довести покрытие кода юнит-тестами до 80%.
3. Доработать синхронизацию:
...
4. Провести оптимизацию кода в части работ с ошибками.

### Для клиента

1. Добавить возможность удаления группы.
2. Добавить возможность удаления e-mail группы.
3. Добавить возможность удаления записи данных.
4. Добавить возможность редактирования записи данных.
5. Доработать в части обеспечения безопасности.

### Для сервера

1. Добавить возможность конфигурирования через флаги, переменные окружения, JSON-файл.
2. Добавить поддержку SQLite.
